...
class MultiMaeForPretraining(nn.Module):
    ...
    def forward_loss(self, x, pred, mask, interpolate_pos_encoding: bool = False):
        ...
        for i, modality in enumerate(self.modalities):
            ...
            loss += __forward_loss(self, x[modality], self.encoders['modality'], pred, mask, interpolate_pos_encoding)
            prev_patches_number += modality_pathces_number
        return loss    
        
    def forward(self, x: Dict[str, torch.FloatTensor] = None, noise: Optional[torch.FloatTensor] = None,
