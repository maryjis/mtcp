...
class DNAmPreprocessor(RNAPreprocessor):
    ...
    def __init__(self, data_train :pd.DataFrame, dataset_dir : Path,
                 ...
                 scaling_method= None, scaling_params = {}, project_ids = [],columns_file_path="src/data/columns.json"):
        super().__init__(data_train, dataset_dir, n_intervals, scaling_method,
                        scaling_params, var_threshold, 
                        is_cluster_genes,threshold,
                        is_hierarchical_cluster,project_ids,
                        columns_file_path)
        ...
        if var_threshold>=0:
            ...
            self.pipe =Pipeline(steps= [('var' , ColumnSelectorFromFile(columns_file_path))])
